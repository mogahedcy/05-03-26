...
export default function RestorationClient() {
    ...
    const servicesList = [
        t('restorationService1' as any),
        t('restorationService2' as any),
        t('restorationService3' as any),
        t('restorationService4' as any),
    ];

    const features = [
        { title: t('rFeature1Title' as any), desc: t('rFeature1Desc' as any) },
        { title: t('rFeature2Title' as any), desc: t('rFeature2Desc' as any) },
        { title: t('rFeature3Title' as any), desc: t('rFeature3Desc' as any) },
        { title: t('rFeature4Title' as any), desc: t('rFeature4Desc' as any) },
    ];
        ...
        <main className="min-h-screen bg-slate-50 relative">
            ...
            <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0">
                    <Image
                        src="/images/trmim/restoration-hero.webp"
                        alt={t('restorationPageTitle' as any)}
                        fill
                ...
                <div className="relative z-10 text-center text-white px-4">
                    <h1 className="text-4xl md:text-6xl font-bold mb-4">{t('restorationPageTitle' as any)}</h1>
                    <p className="text-xl md:text-2xl text-slate-200 max-w-2xl mx-auto">
                        {t('restorationHeroSubtitle' as any)}
                    </p>
                </div>
            </section>
            ...
            <div className="container mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <aside className="lg:col-span-1 space-y-8 h-fit lg:sticky lg:top-24">
                    <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                        <h3 className="text-xl font-bold text-brand-primary mb-4 border-b pb-2">{t('restorationServicesListTitle' as any)}</h3>
                ...
                <article className="lg:col-span-2 prose prose-lg prose-slate max-w-none">
                    <h2 className="text-3xl font-bold text-brand-primary mb-6">{t('restorationWhyTitle' as any)}</h2>
                    <p className="text-slate-600 leading-relaxed mb-8">
                        {t('restorationWhyDesc' as any)}
                    </p>
                    ...
                    <h2 className="text-3xl font-bold text-brand-primary mb-6">معرض صور الترميم والتشطيب</h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
                        {[
                            ...
                            <div key={idx} className="relative h-48 md:h-64 rounded-xl overflow-hidden group shadow-md border border-slate-100">
                                <Image
                                    src={src}
                                    alt={`ترميم وتشطيب مباني جدة - صورة ${idx + 1}`}
                                    fill
                    ...
                    <h2 className="text-2xl font-bold text-brand-primary mb-6">{t('rTypesTitle' as any)}</h2>

                    <div className="grid grid-cols-1 gap-8">
                        {/* Facade Restoration */}
                        <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                            <div className="relative h-64 w-full group overflow-hidden">
                                <Image src="/images/trmim/restoration-exterior.webp" alt={t('rType1Title' as any)} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                            </div>
                            <div className="p-6">
                                <h3 className="text-xl font-bold mb-2">{t('rType1Title' as any)}</h3>
                                <p className="text-slate-600">{t('rType1Desc' as any)}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                            <div className="relative h-64 w-full group overflow-hidden">
                                <Image src="/images/trmim/restoration-interior.webp" alt={t('rType2Title' as any)} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                            </div>
                            <div className="p-6">
                                <h3 className="text-xl font-bold mb-2">{t('rType2Title' as any)}</h3>
                                <p className="text-slate-600">{t('rType2Desc' as any)}</p>
                            </div>
                        </div>
                    </div>
